...
export default function TopRestaurants() {
  ...
  return (
    <section className="pt-16 max-sm:pt-8 pb-24 px-4 md:px-8 lg:px-16 relative bg-zinc-900">
      ...
      <div ref={ref} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-sm:gap-3 max-w-6xl mx-auto">
        {restaurants.map((restaurant, index) => (
          <motion.div
            ...
            className="flex items-center justify-center h-14 gap-2 pl-4 pr-3 leading-none rounded-xl overflow-hidden hover:scale-105 duration-300 transition-all group cursor-pointer bg-zinc-700 shadow-sm hover:shadow-md"
          >

            <div className="flex-1">
              <h3 className="font-semibold text-md max-sm:text-sm text-left text-zinc-100 group-hover:text-[#F24957] transition-colors whitespace-wrap text-wrap">
                {restaurant.name}
              </h3>
            </div>
            <div className="flex items-center gap-1 bg-[#f24957]/80 text-white font-semibold rounded-lg p-2 shadow-sm">
        ...
        >
          <div className="text-center">
            <span className="font-medium text-2xl md:text-2xl max-sm:text-[16px] text-zinc-100">
              E Muito Mais!
            </span>
          </div>
        </motion.div>
